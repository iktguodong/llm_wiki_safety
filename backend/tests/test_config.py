...
def test_load_config_removes_mock_provider_and_resets_default_models(monkeypatch, tmp_path):
    config_dir = tmp_path / ".anniu"
    config_dir.mkdir()
    config_file = config_dir / "config.json"
    config_file.write_text(
                    ...
                    "providers": [
                        {
                            ...
                            "api_key": "test",
                            "models": [
                                {"id": "mock-model", "name": "Mock Model", "type": "chat"},
                            ],
                        },
                        {
                            "id": "deepseek",
    ...
    assert [provider["id"] for provider in loaded["models"]["providers"]] == ["deepseek", "silicon", "bailian"]
    assert loaded["models"]["providers"][0]["api_key"] == "sk-user"
    assert loaded["current_model_id"] == "deepseek-v4-flash"
    assert loaded["models"]["model_roles"] == {
        "doc_parse": "deepseek-v4-flash",
        "qa_chat": "deepseek-v4-flash",
        "ppt_gen": "deepseek-v4-flash",
    }
